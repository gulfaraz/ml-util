...
module.exports = function (f, X, options, P1, P2, P3, P4, P5) {
    ...
    var d1 = mathjs.multiply(mathjs.multiply(-1, mathjs.transpose(s)), s);
    ...
    while(i < mathjs.abs(length)) {
        ...
        mathjs.forEach(z1ProductS, function (value, index, matrix) {
            z1ProductSLength++;
        });
        z1ProductS = mathjs.reshape(z1ProductS, [ z1ProductSLength, 1 ]);
        ...
        var M = mathjs.min(CONSTANT.MAX, mathjs.subtract(mathjs.multiply(-1, length), i));
        if(length > 0) {
            M = CONSTANT.MAX;
        }
        var success = 0;
        var limit = -1;
        while(1) {
            while(((mathjs.larger(f2, mathjs.add(f1, mathjs.multiply(mathjs.multiply(z1, CONSTANT.RHO), d1)))) || (mathjs.larger(d2, mathjs.multiply(-1, CONSTANT.SIG, d1)))) && (M > 0)) {
                limit = mathjs.clone(z1);
                var z2;
                if(mathjs.larger(f2, f1)) {
                    z2 = mathjs.subtract(z3, mathjs.divide(mathjs.multiply(0.5, d3, z3, z3), mathjs.add(mathjs.subtract(mathjs.multiply(d3, z3), f3), f2)));
                } else {
                    ...
                    z2 = mathjs.divide(mathjs.subtract(mathjs.sqrt(mathjs.subtract(mathjs.multiply(B, B), mathjs.multiply(mathjs.multiply(mathjs.multiply(A, d2), z3), z3))), B), A);
                }
                if(mathjs.isNaN(z2) || !mathjs.isFinite(z2)) {
                    z2 = mathjs.divide(z3, 2);
                }
                z2 = mathjs.max(mathjs.min(mathjs.sum(z2), mathjs.sum(mathjs.multiply(CONSTANT.INT, z3))), mathjs.sum(mathjs.multiply(mathjs.subtract(1, CONSTANT.INT), z3)));
                z1 = mathjs.add(z1, z2);
                var z2ProductS = mathjs.multiply(z2, s);
                var z2ProductSLength = 0;
                mathjs.forEach(z2ProductS, function (value, index, matrix) {
                    z2ProductSLength++;
                });
                z2ProductS = mathjs.reshape(z2ProductS, [ z2ProductSLength, 1 ]);
            ...
            if(mathjs.larger(f2, mathjs.add(f1, mathjs.multiply(mathjs.multiply(z1, CONSTANT.RHO), d1))) || mathjs.larger(d2, mathjs.multiply(-1, CONSTANT.SIG, d1))) {
                break;
            } else if(mathjs.larger(d2, mathjs.multiply(CONSTANT.SIG, d1))) {
                success = 1;
                break;
            } else if(M === 0) {
                break;
        ...
        if(success) {
            f1 = mathjs.clone(f2);
            fX = mathjs.transpose(mathjs.concat(mathjs.transpose(fX), [ f1 ], 0));
            ...
            if(mathjs.larger(d2, 0)) {
                ...
                d2 = mathjs.multiply(mathjs.multiply(-1, mathjs.transpose(s)), s);
            }
            z1 = mathjs.multiply(z1, mathjs.min(CONSTANT.RATIO, mathjs.divide(d1, mathjs.subtract(d2, Number.MIN_VALUE))));
            d1 = mathjs.clone(d2);
            lsFailed = 0;
        } else {
            console.log("FAILURE");
            ...
            df2 = mathjs.clone(tmp);
            s = mathjs.multiply(-1, df1);
            d1 = mathjs.multiply(mathjs.multiply(-1, mathjs.transpose(s)), s);
